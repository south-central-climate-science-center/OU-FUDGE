...
for (i in 1:length(rp$predictor.vars)){
  ...
  tmp.hist <- ReadNC(nc.object, var.name=p.var, dstart=start.clip, dcount=count.clip)
  ...
  nc.object <- nc_open(filename) 
  tmp.fut <- ReadNC(nc.object, var.name=p.var, dstart=start.clip, dcount=count.clip, dim='temporal')
  if(rp$apply.spat.mask){
