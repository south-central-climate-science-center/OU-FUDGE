...
adjustWetDays <- function(){

  # Only operates on precip as the target variable
  # Adjustment logic would need to change for precip as one of many predictor vars
  if(rp$target.var=='pr'){
    ...
    lopt.drizzle <- rp$pr.adj.args.outer[2]
    ...
    ref.data.units=attr(ref.data, "units")$value
    wetday.convert <- convert.threshold(threshold, ref.data.units)
    ref.wetdays <- ref.data > wetday.convert
    
    for(i in 1:length(rp$predictor.vars)){
      if(rp$predictor.vars[[i]]=='pr'){
        #Initialize the adjusted reference vectors
        adjust.data.units=attr(adjust.data, "units")$value
        adjust.future.units=attr(adjust.future, "units")$value
        adjust.wetdays <- MaskPRSeries(adjust.data, adjust.data.units, threshold)
        future.wetdays <- MaskPRSeries(adjust.future, adjust.future.units, threshold)    
      }
    }
          ...
          if (length(loop.ref)!=0){ #Avoid running if all NA values present
            ...
            if (fraction.wet.adj > fraction.wet.ref) {
              ...
              threshold.wetday.adj <- quantile(loop.adj, names = FALSE,
                                               ...
                                               na.rm=TRUE)
              print(paste("threshold:", threshold.wetday.adj, " ", i,j))
